...
function MedicationGeminiCalculator() {
  ...
  // Base de datos local de medicamentos comunes para respaldo
  const localMedications = {
    "paracetamol": {
      name: "Paracetamol (Acetaminofén)",
      category: "B",
      description: "Analgésico y antipirético",
      risk: "Los estudios no han demostrado riesgo para el feto. Se considera seguro durante el embarazo a dosis terapéuticas.",
    ...
    "ibuprofeno": {
      ...
      risk: "Categoría C en 1er y 2do trimestre, D en 3er trimestre. Puede causar cierre prematuro del conducto arterioso y otras complicaciones en el tercer trimestre.",
    ...
    "aspirina": {
      ...
      risk: "Puede aumentar el riesgo de sangrado durante el parto. En dosis altas puede asociarse con bajo peso al nacer.",
    ...
    "amoxicilina": {
      ...
      risk: "No se han documentado efectos adversos significativos en el feto.",
      recommendation: "Es uno de los antibióticos de elección durante el embarazo cuando está indicado."
    }
  };

  // Función para buscar en OpenFDA API
  const searchMedications = async () => {
    ...
    try {
      // Buscar primero en la API de OpenFDA
      ...
      if (response.data.medications && response.data.medications.length > 0) {
        setResults(response.data.medications);
      } else {
        // Si no hay resultados, buscar en la base de datos local
        const localResults = searchLocalMedications();
        if (localResults.length > 0) {
          setResults(localResults.map(med => ({
            ...med,
            isLocal: true
          })));
        } else {
      ...
      // En caso de error, intentar buscar en la base de datos local
      const localResults = searchLocalMedications();
      if (localResults.length > 0) {
        setResults(localResults.map(med => ({
          ...med,
  ...
  const searchLocalMedications = () => {
    const normalizedTerm = searchTerm.toLowerCase().trim();
    
    return Object.entries(localMedications)
      .filter(([key, med]) => 
              ...
              <ul className="divide-y divide-gray-200">
                {results.map((med, index) => (
                  ...
                  >
                    <span>{med.name}</span>
                    {med.isLocal && (
                      <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">Base local</span>
                    )}
                  </li>
                ))}
              </ul>
        ...
        {selectedMedication && (
          <div className="mt-6 border rounded-lg overflow-hidden shadow-md animate-fadeIn">
            ...
            <div className="p-5">
              ...
              {fdaCategories[selectedMedication.category] && (
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-medium text-gray-700 mb-2">Descripción de la categoría</h4>
                  <p className="text-gray-600">{fdaCategories[selectedMedication.category]}</p>
                </div>
              )}
        ...
        {!searchTerm && !selectedMedication && (
          <div className="mt-2">
            ...
            <div className="mt-6 bg-blue-50 rounded-lg p-4">
              <h4 className="font-medium text-blue-800 mb-2">Búsqueda de medicamentos</h4>
              <p className="text-sm text-blue-700">
                Esta herramienta utiliza la base de datos de la FDA para proporcionar información sobre la seguridad
                de los medicamentos durante el embarazo. Ingrese el nombre de un medicamento en el campo de búsqueda
                para consultar su categoría y recomendaciones.
              </p>
            </div>
          </div>
        )}
